...
for image, label in zip(src_images, src_labels):
    ...
    for i in range(4):
        slice_image = image[:, i*slice_width:(i+1)*slice_width]
...
for digit in range(10):
    ...
    axs[digit].plot(range(1, 5), avg_counts, marker='o', color='royalblue')
...
plt.suptitle('Average Non-Zero Pixels per Vertical Slice for Each Digit')
